...
static struct write_struct write_task;
...
{
	struct write_struct *task;
	task = container_of(work, struct write_struct, work);
	task->write_emmc_func();
}
