...
static inline int arch_audio_vbc_switch(int master)
{
	...
	case AUDIO_TO_AP_ARM_CTRL:
		...
		#else
		/* Capture will only use AP DMA for the limitation of IRAM size */
		val |=
			(BITS_VBC_DA01_DMA_SYS_SEL(2) | BITS_VBC_AD01_DMA_SYS_SEL(0)
		    | BITS_VBC_AD23_DMA_SYS_SEL(0));
		#endif
...
{
#if defined(REG_AON_APB_RPLL_CFG3) && defined(BITS_AUD_TUNED_26M) && defined(BITS_RPLL_CFG3_MUX1)
	int val;
	int mask = BITS_AUD_TUNED_26M | BITS_RPLL_CFG3_MUX1;

	pr_debug("%s tuned: %d, 9832: %d\n", __func__, tuned,
		 soc_is_scx9832a_v0());
	if (!soc_is_scx9832a_v0())
		return 0;

	val = tuned ? mask : 0;
	sci_glb_write(REG_AON_APB_RPLL_CFG3, val, mask);
	mask = BITS_PLL_PROBE_SEL(0x3f);
	val = tuned ? BITS_PLL_PROBE_SEL(0x20) : 0;
	sci_glb_write(REG_AON_APB_REC_26MHZ_BUF_CFG, val, mask);
#endif
