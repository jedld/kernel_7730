					...
					struct dcam_info   *info)
{
	...
	path->is_work = 0;
	path->end_sel.reserved0 = DCAM_ENDIAN_MAX;
	path->end_sel.reserved1 = DCAM_ENDIAN_MAX;

	switch (fourcc) {
	case IMG_PIX_FMT_GREY:
		path->out_fmt = info->is_loose; // 0 - word/packet, 1 - half word
		path->end_sel.reserved0 = DCAM_ENDIAN_LITTLE;//1;
		path->end_sel.reserved1 = DCAM_ENDIAN_HALFBIG;//2;
		break;
